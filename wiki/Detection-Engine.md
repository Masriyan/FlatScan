...
- **Disassembly** — x86/x64 decoding (via the vendored in-tree `internal/x86asm`) to spot API-hashing loops, PEB walks, GetPC stubs, and anti-VM checks.
...
- **Named-family fingerprints** — matches distinctive markers of known families: RedLine, LummaC2, StealC, Vidar, Raccoon, Agent Tesla, FormBook, XLoader, AsyncRAT, Quasar, Remcos, XWorm, njRAT, and more. A named attribution requires **both** a family-name marker *and* at least one corroborating evidence group (C2 pattern, loot paths, operator strings), so neither a bare family name in an AV report nor generic stealer behavior alone will name a family. The practical consequence: a **packed or obfuscated** sample whose family name is not recoverable from strings falls back to the generic buckets ("Information stealer", "Remote access trojan") rather than being named — the attribution is withheld rather than guessed. When the false-positive guard identifies the file as a signature set or analysis tool, named attribution is withdrawn entirely and listed under `benign_context.suppressed_families`.
